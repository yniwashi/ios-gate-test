...
// - Use a dark border for white ECG lead markers so RA remains visible.
// - Make Visual Guides option buttons responsive and expand single leftover buttons to full row width.
// - Stop replaying placement animations when a lead or pad is tapped; marker taps now only update the placement guide.
// - Remove off-body Mason-Likar lead movement by making all Mason-Likar markers static.
// - Match Mason-Likar chest lead coordinates, image zoom, and lead sizes to 12-Lead while preserving limb lead positions.
// - Match 12-Lead ECG coordinates, image zoom, and lead sizes to the tuned Right-Sided placement and remove sequence controls.
// - Make moving ECG lead animation delay adjustable and pause V4 before it moves to V4R.
// - Add explicit image zoom tuning values for Right-Sided, 12-Lead, and Mason-Likar ECG views.
// - Center ECG lead text inside circular markers with a dedicated label wrapper.
...
const movingElectrode = (id,label,description,startX,startY,x,y,step,color,width=0.05,labelSize=7.5,startLabel="",delay=900) => ({
  id,label,description,x,y,startX,startY,width,rotation:0,color,group:"",kind:"lead",image:"",step,animate:true,labelSize,startLabel,delay
});
...
const guideData = {
  ...
  //
  // Scene call format:
  // scene(title, side, markers, optionalImageZoom, optionalImageOffsetX, optionalImageOffsetY)
  // - optionalImageZoom: zooms the body image and markers together. Example 1.15 = 15% larger.
  // - optionalImageOffsetX: moves the zoomed image and markers together. Positive = right.
  // - optionalImageOffsetY: moves the zoomed image and markers together. Positive = down.
  //
  // Pad call format:
  ...
  // movingElectrode(id, label, description, startX, startY, x, y, step, color, width, labelSize, startLabel, optionalDelayMs)
  // - optionalDelayMs: how long the marker waits before it moves. Example 1600 = 1.6 seconds.
  ...
  pads:{
    ...
    options:[
      { id:"ap", label:"Anterior-Posterior", title:"Anterior-Posterior", color:"#15803D", scenes:[
        ...
        scene("Anterior","front",[pad("adult_ap_anterior","pad_heart.png","Anterior pad","Anterior chest over the cardiac apex/precordium, between the midline and the left nipple.",0.56,0.455,0.08,0.90,0.19,0,C.green)]),
        scene("Posterior","back",[pad("adult_ap_posterior","pad.png","Posterior pad","Patient's left back, left of the spine and just below the scapula at heart level.",0.42,0.55,0.12,0.90,0.19,0,C.green)])
      ]},
      { id:"al", label:"Anterior-Lateral", title:"Anterior-Lateral", color:"#B42318", scenes:[
        scene("Anterior-Lateral","front",[
          ...
          pad("adult_al_upper_right","pad.png","Right upper chest","Patient's right upper chest, right of the sternum and just below the clavicle.",0.39,0.405,0.08,0.90,0.19,-90,C.red),
          pad("adult_al_apex","pad_heart.png","Left lateral chest","Patient's left lateral chest on the mid-axillary line, below and lateral to the left nipple. Avoid placing it too anteriorly.",0.63,0.56,0.08,0.90,0.19,90,C.red)
        ])
      ]},
      { id:"dsed", label:"DSED", title:"DSED", color:"#EA580C", scenes:[
        scene("DSED front view","front",[
          ...
          pad("dsed_set1_upper_right","pad.png","AL - right upper chest","AL pad on the patient's right upper chest, right of the sternum and just below the clavicle.",0.39,0.405,0.08,0.90,0.18,-90,C.red,"AL"),
          pad("dsed_set1_apex","pad_heart.png","AL - left lateral chest","AL pad on the patient's left lateral chest on the mid-axillary line, below and lateral to the left nipple.",0.63,0.56,0.08,0.90,0.18,90,C.red,"AL"),
          pad("dsed_set2_anterior","pad_heart.png","AP - anterior chest","AP anterior pad over the cardiac apex/precordium, between the midline and the left nipple.",0.56,0.47,0.08,0.90,0.18,0,C.green,"AP")
        ]),
        // DSED back: AP posterior pad.
        scene("DSED back view","back",[pad("dsed_set2_posterior","pad.png","AP - posterior back","AP posterior pad on the patient's left back, left of the spine and just below the scapula at heart level.",0.42,0.55,0.12,0.90,0.18,0,C.green,"AP")])
      ]}
    ]
  },
  ecg:{
    ...
    options:[
      { id:"posterior", label:"Posterior", title:"Posterior ECG V7-V9", color:"#7C3AED", scenes:[scene("V7-V9","back",[
        ...
        electrode("v7","V7","Uses the V4 lead. Place at the posterior axillary line, same horizontal level as V6.",0.33,0.535,1,C.darkBlue,0.056),
        electrode("v8","V8","Uses the V5 lead. Place below the left scapular tip, same horizontal level as V6.",0.393,0.535,1,C.orange,0.056),
        electrode("v9","V9","Uses the V6 lead. Place in the left paraspinal area, about 2-3 cm lateral to the spine, level with V6.",0.455,0.535,1,C.purple,0.056)
      ])]},
      { id:"right", label:"Right-Sided", title:"Right-Sided ECG", color:"#0F766E", scenes:[scene("V4R","front",[
        ...
        fixedElectrode("right_ra","RA","Right arm electrode. Place on the right upper limb or right shoulder area.",0.16,0.75,C.white,0.05),
        fixedElectrode("right_la","LA","Left arm electrode. Place on the left upper limb or left shoulder area.",0.835,0.75,C.black,0.05),
        fixedElectrode("right_v1","V1","Standard V1 position: 4th intercostal space at the right sternal border.",0.44,0.45,C.red,0.05),
        fixedElectrode("right_v2","V2","Standard V2 position: 4th intercostal space at the left sternal border.",0.548,0.45,C.yellow,0.05),
        fixedElectrode("right_v3","V3","Standard V3 position: midway between V2 and V4.",0.57,0.51,C.green,0.055),
        movingElectrode("v4r","V4R","Move V4 to V4R: 5th right intercostal space at the right mid-clavicular line.",0.595,0.55,0.42,0.55,1,C.darkBlue,0.055,7.5,"V4",1600),
        fixedElectrode("right_v5","V5","Standard V5 position: same horizontal level as V4 at the anterior axillary line.",0.65,0.55,C.orange,0.05),
        fixedElectrode("right_v6","V6","Standard V6 position: same horizontal level as V4 and V5 at the mid-axillary line.",0.70,0.53,C.purple,0.05)
      ],1.35,0,0.1)]},
      { id:"twelve", label:"12-Lead", title:"12-Lead ECG Placement", color:"#2563EB", scenes:[scene("12-Lead","front",[
        // 12-Lead: static placement matches the tuned Right-Sided coordinate reference.
        fixedElectrode("ra","RA","Right arm electrode. Place on the right upper limb or right shoulder area.",0.16,0.75,C.white,0.048),
        fixedElectrode("la","LA","Left arm electrode. Place on the left upper limb or left shoulder area.",0.835,0.75,C.black,0.048),
        fixedElectrode("v1","V1","4th intercostal space at the right sternal border.",0.455,0.45,C.red,0.048),
        fixedElectrode("v2","V2","4th intercostal space at the left sternal border.",0.548,0.45,C.yellow,0.048),
        fixedElectrode("v3","V3","Midway between V2 and V4.",0.57,0.51,C.green,0.048),
        fixedElectrode("v4","V4","5th intercostal space at the mid-clavicular line.",0.61,0.551,C.darkBlue,0.048),
        fixedElectrode("v5","V5","Same horizontal level as V4 at the anterior axillary line.",0.659,0.545,C.orange,0.048),
        fixedElectrode("v6","V6","Same horizontal level as V4 and V5 at the mid-axillary line.",0.70,0.525,C.purple,0.048)
      ],1.35,0,0.1)]},
      { id:"mason", label:"Mason-Likar", title:"Mason-Likar ECG Placement", color:"#B45309", scenes:[scene("Mason-Likar","front",[
        // Mason-Likar: limb leads are moved onto the torso; chest leads match 12-Lead positions.
        fixedElectrode("ml_ra","RA","Mason-Likar RA: right infraclavicular fossa, centered near the deltoid border below the clavicle.",0.35,0.38,C.white),
        fixedElectrode("ml_la","LA","Mason-Likar LA: left infraclavicular fossa, centered near the deltoid border below the clavicle.",0.625,0.38,C.black),
        fixedElectrode("ml_rl","RL","Mason-Likar RL: ground/reference electrode on the lower right torso, above the iliac crest.",0.36,0.71,C.green),
        fixedElectrode("ml_ll","LL","Mason-Likar LL: left anterior axillary line, midway between the lower rib cage and iliac crest.",0.65,0.66,C.orange),
        fixedElectrode("ml_v1","V1","4th intercostal space at the right sternal border.",0.455,0.45,C.red,0.048),
        fixedElectrode("ml_v2","V2","4th intercostal space at the left sternal border.",0.548,0.45,C.yellow,0.048),
        fixedElectrode("ml_v3","V3","Midway between V2 and V4.",0.57,0.51,C.green,0.048),
        fixedElectrode("ml_v4","V4","5th intercostal space at the mid-clavicular line.",0.61,0.551,C.darkBlue,0.048),
        fixedElectrode("ml_v5","V5","Same horizontal level as V4 at the anterior axillary line.",0.659,0.545,C.orange,0.048),
        fixedElectrode("ml_v6","V6","Same horizontal level as V4 and V5 at the mid-axillary line.",0.70,0.525,C.purple,0.048)
      ],1.35,0,0.1)]}
    ]
  },
  burns:{
...
function guideStyles() {
  return `
    ...
    .vg-seg{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:8px}.vg-seg button{grid-column:span 2}.vg-seg.orphan-3 button:last-child,.vg-seg.count-1 button{grid-column:1/-1}.vg-seg button,.vg-chip{min-height:43px;border:1px solid var(--border);border-radius:999px;background:var(--surface-2);color:var(--text);font-weight:950}.vg-seg button.active,.vg-chip.active{border-color:var(--vg-accent);background:color-mix(in oklab,var(--vg-accent) 14%,var(--surface));color:var(--vg-accent)}
    ...
    .vg-marker{position:absolute;left:var(--mx-px,0px);top:var(--my-px,0px);width:var(--mw-px,28px);height:var(--mh-px,28px);transform:translate(-50%,-50%) scale(1);display:grid;place-items:center;border:2px solid var(--bc,#fff);box-shadow:0 5px 12px rgba(15,23,42,.25);z-index:4;transition:left 1100ms cubic-bezier(.2,.8,.2,1),top 1100ms cubic-bezier(.2,.8,.2,1)}
    ...
    .vg-marker.lead{border-radius:50%;background:var(--c);color:var(--tc,#fff);font-size:calc(var(--label-size,7.5) * 1px);font-weight:950;line-height:1;text-align:center;padding:0}
    .vg-lead-label{display:flex;align-items:center;justify-content:center;width:100%;height:100%;line-height:1;transform:translateY(.5px);letter-spacing:-.01em}
    .vg-marker.pad{border:0;border-radius:8px;background:transparent;box-shadow:none}
    ...
    @media(max-width:340px){.vg-scenes,.vg-burn-diagrams{grid-template-columns:1fr}.vg-scenes:not(.single) .vg-body{height:240px}}@media(max-width:520px){.vg{padding:12px 10px}.vg-seg{gap:6px}.vg-seg button{grid-column:span 3;font-size:12px}.vg-seg.orphan-3 button:last-child{grid-column:span 3}.vg-seg.orphan-2 button:last-child,.vg-seg.count-1 button{grid-column:1/-1}}
  `;
}
...
export async function run(root) {
  ...
  function render() {
    ...
    const optionCount = cat.options.length;
    const optionLayoutClass = [
      `count-${optionCount}`,
      optionCount % 3 === 1 ? "orphan-3" : "",
      optionCount % 2 === 1 ? "orphan-2" : ""
    ].filter(Boolean).join(" ");
    app.innerHTML = `
      ...
      <section class="vg-card">
        <div class="vg-seg ${optionLayoutClass}">${cat.options.map((item) => `<button type="button" class="${item.id===optionId?"active":""}" data-option="${item.id}">${esc(item.label)}</button>`).join("")}</div>
      </section>
  ...
  function renderContent() {
    ...
    content.innerHTML = `
      ...
      <div id="vgDetailSlot">${selectedMarker ? renderMarkerDetail(selectedMarker) : ""}</div>
    ...
    content.querySelectorAll(".vg-marker").forEach((button) => {
      button.addEventListener("click", (event) => {
        ...
        const detailSlot = content.querySelector("#vgDetailSlot");
        if (detailSlot) detailSlot.innerHTML = selectedMarker ? renderMarkerDetail(selectedMarker) : "";
      });
    });
    ...
    content.querySelectorAll(".vg-marker[data-animate='true']").forEach((button) => {
      ...
      window.setTimeout(() => {
        ...
        if (button.dataset.finalLabel) {
          const label = button.querySelector(".vg-lead-label");
          if (label) label.textContent = button.dataset.finalLabel;
          else button.textContent = button.dataset.finalLabel;
        }
      }, delay);
    });
  }
  ...
  function renderMarker(item, sceneIndex) {
    ...
    const borderColor = item.kind === "lead" && item.color === C.white ? C.black : C.white;
    const leadText = esc(item.startLabel && animate ? item.startLabel : item.label);
    const content = item.kind === "pad" && item.image
      ? `<img class="vg-pad-img" src="images/visual_guides/${esc(item.image)}" alt="">`
      : `<span class="vg-lead-label">${leadText}</span>`;
